...
class EchoAR{
    let api_key="" //insert your echoAR API key here
    //echoar endpoint
    ...
    func parseEntry(key:String, entry:Entry){
        if(entry.filename.contains(".obj")){
    ...
    /// loads SCNNode from fileNames and return list of SCCNode
    /// - modelNames: Set of 3-D model names you want load nodes from
    /// - completion Handler
    func loadNodesFromNames(modelNames:Set<String>, completion: ([SCNNode]) -> ()) {
        self.queryDatabase(api_key: api_key, completion: { (entries) in
            var ind = 0
    ...
    func loadNodeFromIndex(index:Int, completion: (SCNNode) -> ()){
        loadSceneAtIndex(index: index) { (scene) in
            ...
            guard let node = scene.rootNode.childNode(withName: name, recursively: true) else {return completion(SCNNode())}
            completion(node)
        }
    }
